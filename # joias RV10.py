...
# RV8 ajustado: 29/11/24 , ajustar a logica para combinações entre formas geometricas 
# idependente da cor das formas geometricas  - RV10
# ajustado, mas ficou muito facil
...
FORMAS_JOIAS = ['quadrado', 'triangulo', 'circulo']
...
def verificar_combinacoes():
    ...
    for y in range(linhas):
        for x in range(colunas - 2):
            if tabuleiro[y][x][1] == tabuleiro[y][x + 1][1] == tabuleiro[y][x + 2][1]:
                combinacoes.update([(x, y), (x + 1, y), (x + 2, y)])

    # Verificar combinações verticais
    for x in range(colunas):
        for y in range(linhas - 2):
            if tabuleiro[y][x][1] == tabuleiro[y + 1][x][1] == tabuleiro[y + 2][x][1]:
